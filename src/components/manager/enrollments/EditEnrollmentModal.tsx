...
export default function EditEnrollmentModal({
  ...
  const [selectedClass, setSelectedClass] = useState(enrollment.class?._id ?? '');
  const [selectedCourse, setSelectedCourse] = useState(enrollment.course?._id ?? '');
  ...
  const initials = `${enrollment.student?.firstName?.charAt(0) ?? ''}${enrollment.student?.lastName?.charAt(0) ?? ''}`.toUpperCase();
      ...
      <div className="relative w-full max-w-md bg-[#181826] rounded-2xl shadow-[0_24px_80px_rgba(0,0,0,0.7)] border border-[rgba(71,71,84,0.3)] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[rgba(71,71,84,0.25)]">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-gradient-to-br from-[rgba(182,160,255,0.2)] to-[rgba(104,250,221,0.1)] border border-[rgba(182,160,255,0.2)]">
              <Pencil size={15} className="text-[#b6a0ff]" />
            </div>
            <div>
              <h2 className="text-base font-bold text-[#e9e6f7] leading-none">Edit Enrollment</h2>
              <p className="text-[11px] text-[#aba9b9] mt-0.5">Update class, course, or status</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex flex-col gap-5">
          ...
          <div className="flex items-center gap-3.5 px-4 py-3.5 rounded-xl bg-[#12121e] border border-[rgba(71,71,84,0.25)]">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-[#e9e6f7] truncate">{studentName || 'Unknown Student'}</div>
              <div className="text-[11px] text-[#aba9b9] truncate">{enrollment.student?.email}</div>
            </div>
        ...
        <div className="px-6 pb-6 pt-1 flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-[#0d0d18] transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:-translate-y-px hover:shadow-[0_10px_30px_rgba(104,250,221,0.25)] flex items-center justify-center gap-2"
            style={{ background: 'linear-gradient(135deg,#b6a0ff,#68fadd)' }}
          >
            {isLoading && <div className="w-3.5 h-3.5 rounded-full border-2 border-[#0d0d18]/30 border-t-[#0d0d18] animate-spin" />}
