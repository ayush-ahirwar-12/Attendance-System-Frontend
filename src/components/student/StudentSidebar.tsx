...
import {
  LayoutDashboard, BookOpen, ScanFace,
...
const navItems = [
  ...
  { id: 'attendance', label: 'Mark Attendance', icon: <ClipboardCheck size={18} />, href: '/' },
  { id: 'courses', label: 'My Courses', icon: <BookOpen size={18} />, href: '/' },
];

export default function StudentSidebar({ user }: { user: User }) {
  ...
  return (
    <aside className="w-[260px] shrink-0 flex flex-col bg-[#12121e] px-4 py-6 sticky top-0 h-screen overflow-y-auto border-r border-[rgba(182,160,255,0.06)]">
      ...
      <div className="flex items-center gap-3 px-3 pb-7">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#b6a0ff] to-[#7e51ff] shadow-[0_4px_20px_rgba(126,81,255,0.4)]">
      ...
      <div className="border-t border-[rgba(71,71,84,0.4)] pt-4 mt-4 flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-[11px] text-[#aba9b9] capitalize">{user?.role}</div>
        </div>
      </div>
    </aside>
  );
